...
class TaskAlignedAssigner_test(nn.Module):
    """
    A task-aligned assigner for object detection.
    ...
    def _forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        target_scores : (b, h*w, num_classes) target scores for positive anchor points, with shape (b, h*w, num_classes).
        """
        target_labels, target_bboxes, target_scores = self.get_targets_full(gt_labels, gt_bboxes, mask_pos, fg_mask)
    ...
    def get_targets_full(self, gt_labels, gt_bboxes, target_weights, fg_mask):
        """
        Compute soft target labels, boxes, and scores using a weight matrix.
