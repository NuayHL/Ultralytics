...
def generate_parameter_grid(search_space):
    ...
    scale_cfg = search_space['scale']

    scale_range = np.arange(scale_cfg['min'], scale_cfg['max'] + scale_cfg['step'], scale_cfg['step'])
    grid = []
    for scale in scale_range:
        # Round to handle potential float precision issues
        grid.append(round(scale, 4))
...
def initialize_progress(progress_file, grid):
    ...
    if os.path.exists(progress_file):
        ...
        progress = {f"s{s}": {"scale": s, "status": "pending"} for s in grid}
...
def generate_model_config(base_config_path, output_path, scale):
    ...
    model_config['assigner_type'] = 'TaskAlignedAssigner_Scale'
    model_config['topk'] = 10
    model_config['alpha'] = 0.5
    model_config['beta'] = 6.0
    model_config['scale_ratio'] = scale
    # assigner_type: 'TaskAlignedAssigner_Scale'
    # topk: 10
    # alpha: 0.5
    # beta: 6.0
    # scale_ratio: 2
...
def main():
    ...
    for exp_key, details in tqdm(progress_data.items(), desc="Total Experiments"):
        ...
        try:
            ...
            exp_run_name = f"{train_params['run_name_prefix']}_s{scale}"
            ...
            tqdm.write(f"\n---> STARTING: {exp_key} | scale={scale}")
            tqdm.write(f"  - Generating config: {generated_config_path}")
            generate_model_config(paths['base_model_config'], generated_config_path, scale)
