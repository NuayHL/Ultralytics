...
def plot_epoch_change(csv_sources, out_dir=None, metric="mean_jaccard", bin_name=None):
    """
    Plot line charts for epoch change stats from multiple CSV files.

    Args:
        csv_sources (str, list, or dict):
            - str: Path to a single CSV file.
            - list: List of CSV file paths. Legend labels will be filenames.
            - dict: Key is the legend label (e.g., "Baseline"), Value is the CSV path.
        out_dir (str, optional): Output directory. Defaults to the directory of the first CSV.
        metric (str): Metric to plot ('mean_jaccard' or 'mean_change_ratio').
        bin_name (str, optional): Filter for a specific 'area_bin'. If None, plots all bins.

    Returns:
        str: Path to the saved image.
    """

    # 1. Standardize input to a dictionary: {Label: Path}
    sources_dict = {}

    if isinstance(csv_sources, str):
        # Single path
        if not os.path.isfile(csv_sources):
            raise FileNotFoundError(f"CSV not found: {csv_sources}")
        sources_dict["Main"] = csv_sources
        first_path = csv_sources

    elif isinstance(csv_sources, list):
        # List of paths
        for idx, path in enumerate(csv_sources):
            if not os.path.isfile(path):
                raise FileNotFoundError(f"CSV not found: {path}")
            # Prefer parent directory name as label to avoid collisions when filenames are identical
            parent_label = os.path.basename(os.path.dirname(path)) or "source"
            base_label = os.path.splitext(os.path.basename(path))[0]
            label = parent_label if parent_label else base_label
            # Ensure uniqueness (e.g., multiple files in different dirs but same parent)
            if label in sources_dict:
                label = f"{label}_{idx}"
            sources_dict[label] = path
        first_path = csv_sources[0]

    elif isinstance(csv_sources, dict):
        # Dictionary {Label: Path}
        for label, path in csv_sources.items():
            if not os.path.isfile(path):
                raise FileNotFoundError(f"CSV not found: {path}")
            sources_dict[label] = path
        first_path = list(csv_sources.values())[0]
    else:
        raise TypeError("csv_sources must be a str, list, or dict.")

    # 2. Check metric
    if metric not in {"mean_jaccard", "mean_change_ratio"}:
        raise ValueError("metric support only 'mean_jaccard' or 'mean_change_ratio'")
    ...
    if bin_name is not None:
        ...
        if df.empty:
            raise ValueError(f"No data found for bin_name='{bin_name}'")
        bins = [bin_name]
    else:
        bins = sorted(df["area_bin"].unique().tolist())

    # 6. Plotting
    plt.figure(figsize=(10, 6))  # Increased width slightly for legend

    # Loop through each bin
    for b_name in bins:
        bin_data = df[df["area_bin"] == b_name]

        # Loop through each source within this bin to maintain line continuity
        # Sort by source to keep legend order consistent
        unique_sources = sorted(bin_data['source'].unique())

        for src in unique_sources:
            sub = bin_data[bin_data["source"] == src].sort_values("epoch_j")
            if sub.empty:
                continue
            ...
            # Label format: "BinName - SourceLabel" (or just "BinName" if only 1 source)
            if len(sources_dict) > 1:
                label_str = f"{b_name} ({src})"
            else:
                label_str = f"{b_name}"

            plt.plot(x, y, marker="o", markersize=4, label=label_str)
    ...
    # Adjust legend position if too many lines
    if len(bins) * len(sources_dict) > 5:
    ...
    # 7. Save
    suffix = "_multi_compare" if len(sources_dict) > 1 else ""
    ...
    plt.savefig(out_path, dpi=300, bbox_inches='tight')  # bbox_inches prevents legend cut-off
    plt.close()
...
def plot_epoch_change_for_paper(
    csv_sources,
    out_dir=None,
    metric="mean_jaccard",
    bin_name=None,
    ...
    """
    Plot line charts for epoch change stats (paper-ready, single-column).

    Args:
        csv_sources (str | list | dict):
            - str: single CSV path
            - list: list of CSV paths
            - dict: {source_key: csv_path}
        out_dir (str): output directory
        metric (str): 'mean_jaccard' or 'mean_change_ratio'
        bin_name (str | None): area_bin to plot
        label_map (dict | None):
            Custom legend labels, e.g.
            {
                "baseline": "CIoU",
                "ours": "Stability-Oriented Metric"
            }
        figsize (tuple): figure size for single-column layout
    ...
    if isinstance(csv_sources, str):
        ...
        for i, p in enumerate(csv_sources):
            key = f"src{i}"
            sources_dict[key] = p
        first_path = csv_sources[0]

    elif isinstance(csv_sources, dict):
        sources_dict = csv_sources
        first_path = list(csv_sources.values())[0]

    else:
        raise TypeError("csv_sources must be str, list, or dict")

    # ------------------------------------------------------------------
    # 2. Metric check
    # ------------------------------------------------------------------
    if metric not in {"mean_jaccard", "mean_change_ratio"}:
        raise ValueError("metric must be 'mean_jaccard' or 'mean_change_ratio'")

    # ------------------------------------------------------------------
    # 3. Load data
    # ------------------------------------------------------------------
    dfs = []
    for key, path in sources_dict.items():
        df = pd.read_csv(path)
        df["source"] = key
        dfs.append(df)
    ...
    if bin_name is not None:
        df = df[df["area_bin"] == bin_name]
        if df.empty:
            raise ValueError(f"No data for area_bin={bin_name}")

    # ------------------------------------------------------------------
    # 6. Plot (single-column, paper style)
    # ------------------------------------------------------------------
    fig, ax = plt.subplots(figsize=figsize)

    for key in sources_dict.keys():
        sub = df[df["source"] == key].sort_values("epoch_j")
        if sub.empty:
            continue
        ...
        # Custom label
        if label_map is not None and key in label_map:
            label = label_map[key]
        else:
            label = key
    ...
    # 8. Save
    # ------------------------------------------------------------------
    suffix = "_multi" if len(sources_dict) > 1 else ""
    bin_suffix = "" if bin_name is None else f"_{bin_name}"
    out_path = os.path.join(
        out_dir, f"{metric}_epoch_curve{bin_suffix}{suffix}.png"
    )
...
if __name__ == "__main__":
    ...
    bin_name = ["< 64", "64 - 256", "256 - 1024", "Overall"]
    
    # plot_epoch_change(
    #     csv_sources=[os.path.join("..",_SAVE_DIR, "v12s_record_stability_stats", "epoch_change_stats.csv"),
    #                  os.path.join("..",_SAVE_DIR, "v12s_record_hausdorff_stability_stats", "epoch_change_stats.csv")],
    #     out_dir=os.path.join(_SAVE_DIR, "v12s_record_stability_stats"),
    #     metric="mean_change_ratio",
    #     bin_name="Overall"
    # )

    plot_epoch_change_for_paper(
        csv_sources={"baseline":os.path.join("..",_SAVE_DIR, "v12s_record_stability_stats", "epoch_change_stats.csv"),
                     "ours":os.path.join("..",_SAVE_DIR, "v12s_record_hausdorff_stability_stats", "epoch_change_stats.csv")},
        out_dir=os.path.join(_SAVE_DIR, "v12s_record_stability_stats"),
        metric="mean_change_ratio",
        label_map={"ours": "Scale-Aware alignment Metric",
                   "baseline": "CIoU"},
        bin_name="Overall"
    )

    plot_epoch_change_for_paper(
        csv_sources={"baseline":os.path.join("..",_SAVE_DIR, "v12s_record_hituav_stability_stats", "epoch_change_stats.csv"),
                     "ours":os.path.join("..",_SAVE_DIR, "v12s_record_hituav_hausdorff_stability_stats", "epoch_change_stats.csv")},
        out_dir=os.path.join(_SAVE_DIR, "v12s_record_hituav_stability_stats"),
        metric="mean_change_ratio",
        label_map={"ours": "Scale-Aware alignment Metric",
                   "baseline": "CIoU"},
        bin_name="Overall"
    ...
    # process_all_epochs(
    #     model_dir="runs/detect/hituav/v12s_record/weights/",
    #     dataset_path="hit-uav.yaml",
    #     output_dir="v12s_record_hituav_stability",
    #     assign_kwargs={"topk": 10, "num_classes": 5, "alpha": 0.5, "beta": 6},
    #     assigner_type='TaskAlignedAssigner',
    #     project_path='runs_assignment',
    #     compute_stability=False
    # )
    ...
    # process_all_epochs(
    #     model_dir="runs/detect/hituav/v12s_record_hausdorff/weights/",
    #     dataset_path="hit-uav.yaml",
    #     output_dir="v12s_record_hituav_hausdorff_stability",
    #     assign_kwargs={"topk": 10, "num_classes": 5, "alpha": 0.5, "beta": 6},
    #     assigner_type='TaskAlignedAssigner_VaryingIoU_Sep',
    #     project_path='runs_assignment',
    #     compute_stability=False
    ...
    # process_all_epochs(
    #     model_dir="runs/detect/hituav/v12s_record_l1_04/weights/",
    #     dataset_path="hit-uav.yaml",
    #     output_dir="v12s_record_hituav_l1_04_stability",
    #     assign_kwargs={"topk": 10, "num_classes": 5, "alpha": 0.5, "beta": 6},
    #     assigner_type='TaskAlignedAssigner_VaryingIoU_Sep',
    #     project_path='runs_assignment',
    #     compute_stability=False
    ...
    #
    # process_all_epochs(
    #     model_dir="runs/detect/hituav/v12s_record_l1_10/weights/",
    #     dataset_path="hit-uav.yaml",
    #     output_dir="v12s_record_hituav_l1_10_stability",
    #     assign_kwargs={"topk": 10, "num_classes": 5, "alpha": 0.5, "beta": 6},
    #     assigner_type='TaskAlignedAssigner_VaryingIoU_Sep',
    #     project_path='runs_assignment',
    #     compute_stability=False
    ...
    # compute_assignment_stability(
    #     base_dir=os.path.join(_SAVE_DIR, "v12s_record_hituav_stability"),
    #     save_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_stability_stats", "stability_stats.csv"),
    #     save_epoch_changes_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_stability_stats", "epoch_change_stats.csv"),
    #     area_thresholds=[8*8, 16*16, 32.0 * 32.0, 64.0 * 64.0]
    # )

    # compute_assignment_stability(
    #     base_dir=os.path.join(_SAVE_DIR, "v12s_record_hituav_hausdorff_stability"),
    #     save_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_hausdorff_stability_stats", "stability_stats.csv"),
    #     save_epoch_changes_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_hausdorff_stability_stats", "epoch_change_stats.csv"),
    #     area_thresholds=[8*8, 16*16, 32.0 * 32.0, 64.0 * 64.0]
    # )

    # compute_assignment_stability(
    #     base_dir=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_04_stability"),
    #     save_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_04_stability_stats", "stability_stats.csv"),
    #     save_epoch_changes_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_04_stability_stats", "epoch_change_stats.csv"),
    #     area_thresholds=[8*8, 16*16, 32.0 * 32.0, 64.0 * 64.0]
    # )
    #
    # compute_assignment_stability(
    #     base_dir=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_10_stability"),
    #     save_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_10_stability_stats", "stability_stats.csv"),
    #     save_epoch_changes_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_10_stability_stats", "epoch_change_stats.csv"),
    #     area_thresholds=[8*8, 16*16, 32.0 * 32.0, 64.0 * 64.0]
    # )

    # plot_epoch_change(
    #     csv_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_stability_stats", "epoch_change_stats.csv"),
    #     out_dir=os.path.join(_SAVE_DIR, "v12s_record_hituav_stability_stats"),
    #     metric="mean_jaccard"
    # )
    # plot_epoch_change(
    #     csv_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_hausdorff_stability_stats", "epoch_change_stats.csv"),
    #     out_dir=os.path.join(_SAVE_DIR, "v12s_record_hituav_hausdorff_stability_stats"),
    #     metric="mean_jaccard"
    # )

    # plot_epoch_change(
    #     csv_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_10_stability_stats", "epoch_change_stats.csv"),
    #     out_dir=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_10_stability_stats"),
    #     metric="mean_jaccard"
    # )
    # plot_epoch_change(
    #     csv_path=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_04_stability_stats", "epoch_change_stats.csv"),
    #     out_dir=os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_04_stability_stats"),
    #     metric="mean_jaccard"
    # )
    ...
    #
    # plot_epoch_change(
    #     csv_sources=[os.path.join(_SAVE_DIR, "v12s_record_hituav_stability_stats", "epoch_change_stats.csv"),
    #                  os.path.join(_SAVE_DIR, "v12s_record_hituav_hausdorff_stability_stats", "epoch_change_stats.csv"),
    #                  os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_04_stability_stats", "epoch_change_stats.csv"),
    #                  os.path.join(_SAVE_DIR, "v12s_record_hituav_l1_10_stability_stats", "epoch_change_stats.csv")],
    #     out_dir=os.path.join(_SAVE_DIR, "overall"),
    #     metric="mean_change_ratio",
    #     bin_name=bin_name[2]
    # )